...
TYPES = {"grow":"grow","shrink":"shrink","squeeze":"squeeze","colorfight":"colorfight","paint":"paint","merge":"merge","king":"king","swarm":"swarm","spawn":"spawn","rings":"rings","race":"race","split":"split","random":"random"}
...
BG=(6,6,12); INK=(240,240,255)
MIN_SECONDS=40.0; MAX_SECONDS=58.0; HOLD_AFTER=2.0
...
def render_simulation(sim_type, duration, cfg, out_root, work_dir, options=None):
    options=options or {}
    ...
    if shape not in SHAPES: shape="circle"
    ...
    info(f"Simulation {sim_type} / {shape}")
...
def _encode(sim_type,shape,speed,palette,fps,W,H,dest,rng):
    ...
    dt=1.0/fps; max_frames=int(MAX_SECONDS*fps); min_frames=int(MIN_SECONDS*fps); hold=int(HOLD_AFTER*fps)
    finished_at=None; hits=[]; last_hit_flag=[0]; frame=0
    try:
        while frame<max_frames:
            before=last_hit_flag[0]
            for _ in range(3): step_fn(dt/3.0,last_hit_flag)
            if last_hit_flag[0]>before: hits.append(frame/fps)
            if finished_at is None and frame>=min_frames and done_fn(): finished_at=frame
            img=Image.new("RGB",(W,H),BG); draw_fn(ImageDraw.Draw(img),W,H); proc.stdin.write(img.tobytes()); frame+=1
            if finished_at is not None and frame-finished_at>=hold: break
    finally:
        proc.stdin.close()
...
def _arena(shape,cx,cy,W,H):
    if shape=="square": return {"kind":"box","box":(90.0,200.0,W-90.0,H-200.0),"R":240}
    if shape=="triangle": return {"kind":"poly","pts":_poly(cx,cy+16,3,248),"R":248}
    if shape=="hexagon": return {"kind":"poly","pts":_poly(cx,cy,6,248),"R":248}
...
def _boost(ball, extra=1.1):
    ball.vx*=extra; ball.vy*=extra
    spd=math.hypot(ball.vx,ball.vy)
    if spd>980:
        ball.vx*=980/spd; ball.vy*=980/spd
    elif spd<300:
        ball.vx*=340/max(spd,1); ball.vy*=340/max(spd,1)
...
def _bounce_arena(ball,arena,cx,cy,hits):
    if arena["kind"]=="circle":
        ...
        if hitting:
            ...
            return fresh
        ball.touching=False; return False
    if arena["kind"]=="box":
        l,t,r,b=arena["box"]; hitting=False
        if ball.x-ball.r<=l: ball.x=l+ball.r+0.4; ball.vx=abs(ball.vx)*1.12; hitting=True
        elif ball.x+ball.r>=r: ball.x=r-ball.r-0.4; ball.vx=-abs(ball.vx)*1.12; hitting=True
        if ball.y-ball.r<=t: ball.y=t+ball.r+0.4; ball.vy=abs(ball.vy)*1.12; hitting=True
        elif ball.y+ball.r>=b: ball.y=b-ball.r-0.4; ball.vy=-abs(ball.vy)*1.12; hitting=True
        if hitting: _boost(ball,1.06)
        fresh=_new_bounce(ball,hitting)
        if fresh: hits[0]+=1
        if not hitting: ball.touching=False
        return fresh
...
def _move(ball,dt):
    ball.x+=ball.vx*dt; ball.y+=ball.vy*dt; ball.trail.append((ball.x,ball.y)); del ball.trail[:-28]
...
def _resolve_circle(ball,cx,cy,R,dx,dy,dist):
    nx,ny=dx/dist,dy/dist; limit=max(2.0,R-ball.r-0.5)
    ball.x=cx+nx*limit; ball.y=cy+ny*limit
    dot=ball.vx*nx+ball.vy*ny
    if dot>0:
        ball.vx-=2.18*dot*nx; ball.vy-=2.18*dot*ny
    _boost(ball)

def _new_bounce(ball,hitting):
    fresh=hitting and not ball.touching; ball.touching=hitting; return fresh
...
def _draw_ball(d,ball):
    ...
    r=ball.r; d.ellipse([ball.x-r,ball.y-r,ball.x+r,ball.y+r],fill=ball.color)
    g=max(2.0,r*0.26); d.ellipse([ball.x-g,ball.y-g-r*0.16,ball.x+g*0.5,ball.y],fill=(255,255,255))

def _mk(cx,cy,rng,palette,speed,r=13,team=0):
    ang=rng.random()*math.tau; spd=rng.uniform(380,560)*speed
    return Ball(cx,cy,math.cos(ang)*spd,math.sin(ang)*spd,r,palette[team%len(palette)],team)

def _spread(cx,cy,rng,palette,speed,n,r=12,teams=3):
    balls=[]
    for i in range(n):
        ang=(i/max(1,n))*math.tau; rad=70+(i%4)*22
        b=_mk(cx+math.cos(ang)*rad, cy+math.sin(ang)*rad, rng,palette,speed,r+(i%3),team=i%teams)
        b.x=cx+math.cos(ang)*rad; b.y=cy+math.sin(ang)*rad; balls.append(b)
    return balls
...
def _collide(balls):
    for i in range(len(balls)):
        for j in range(i+1,len(balls)):
            ...
            dx,dy=b.x-a.x,b.y-a.y; dist=math.hypot(dx,dy) or 0.001; need=a.r+b.r
            if dist>=need: continue
            nx,ny=dx/dist,dy/dist; overlap=need-dist
            a.x-=nx*overlap/2; a.y-=ny*overlap/2; b.x+=nx*overlap/2; b.y+=ny*overlap/2
            va=a.vx*nx+a.vy*ny; vb=b.vx*nx+b.vy*ny
            a.vx+=(vb-va)*nx*1.06; a.vy+=(vb-va)*ny*1.06
            b.vx+=(va-vb)*nx*1.06; b.vy+=(va-vb)*ny*1.06
            yield a,b

def _fresh_pairs(balls,seen):
    now=set(); fresh=[]
    for a,b in _collide(balls):
        key=tuple(sorted((id(a),id(b)))); now.add(key)
        if key not in seen: fresh.append((a,b))
    seen.clear(); seen.update(now); return fresh

def _grow(cx,cy,W,H,shape,speed,palette,rng):
    arena=_arena(shape,cx,cy,W,H); ball=_mk(cx,cy,rng,palette,speed,14); cap=arena.get("R",240)-8
    def step(dt,hits):
        _move(ball,dt)
        if _bounce_arena(ball,arena,cx,cy,hits): ball.r=min(ball.r+2.4,cap)
    return step, lambda d,Ww,Hh: (_draw_arena(d,arena,cx,cy), _draw_ball(d,ball)), lambda: ball.r>=cap-1
...
def _shrink(cx,cy,W,H,shape,speed,palette,rng):
    ...
    return step, lambda d,Ww,Hh: (_draw_arena(d,arena,cx,cy), _draw_ball(d,ball)), lambda: (arena["kind"]=="circle" and R[0]<=ball.r+9) or (arena["kind"]=="box" and arena["box"][2]-arena["box"][0]<ball.r*2+20)
...
def _squeeze(cx,cy,W,H,shape,speed,palette,rng):
    ...
    def step(dt,hits):
        ...
        if _bounce_arena(ball,arena,cx,cy,hits):
            ...
            if arena["kind"]=="circle": R[0]=max(ball.r+7,R[0]-3.0); arena["R"]=R[0]
    return step, lambda d,Ww,Hh: (_draw_arena(d,arena,cx,cy), _draw_ball(d,ball)), lambda: ball.r>=R[0]-8

def _colorfight(cx,cy,W,H,shape,speed,palette,rng):
    arena=_arena(shape,cx,cy,W,H); teams=3 if len(palette)>2 else 2
    balls=_spread(cx,cy,rng,palette,speed,teams*6,r=11,teams=teams); seen=set(); won=[False]
    def step(dt,hits):
        for b in balls: _move(b,dt); _bounce_arena(b,arena,cx,cy,hits)
        for a,b in _fresh_pairs(balls,seen):
            if a.team!=b.team and rng.random()<0.55:
                if rng.random()<0.5: b.team,b.color=a.team,a.color
                else: a.team,a.color=b.team,b.color
        won[0]=len({b.team for b in balls})==1
    def draw(d,Ww,Hh):
        _draw_arena(d,arena,cx,cy)
        for b in balls: _draw_ball(d,b)
...
def _paint(cx,cy,W,H,shape,speed,palette,rng):
    ...
    def step(dt,hits):
        _move(ball,dt)
        if _bounce_arena(ball,arena,cx,cy,hits): ball.color=rng.choice(palette)
        dots.append((ball.x,ball.y,ball.color,ball.r*0.9))
        if len(dots)>420: del dots[:40]
    def draw(d,Ww,Hh):
        for x,y,c,r in dots: d.ellipse([x-r,y-r,x+r,y+r],fill=c)
...
def _merge(cx,cy,W,H,shape,speed,palette,rng):
    arena=_arena(shape,cx,cy,W,H); balls=_spread(cx,cy,rng,palette,speed,12,r=12,teams=3); seen=set()
    def step(dt,hits):
        live=[b for b in balls if b.alive]
        for b in live: _move(b,dt); _bounce_arena(b,arena,cx,cy,hits)
    ...
    def draw(d,Ww,Hh):
        _draw_arena(d,arena,cx,cy)
        for b in balls:
            if b.alive: _draw_ball(d,b)
    return step,draw,lambda: sum(1 for b in balls if b.alive)<=3

def _king(cx,cy,W,H,shape,speed,palette,rng):
    arena=_arena(shape,cx,cy,W,H); balls=_spread(cx,cy,rng,palette,speed,12,r=13,teams=3); seen=set(); grace=[90]
    def step(dt,hits):
        grace[0]=max(0,grace[0]-1); live=[b for b in balls if b.alive]
        for b in live: _move(b,dt); _bounce_arena(b,arena,cx,cy,hits)
        if grace[0]: list(_collide(live)); return
        for a,b in _fresh_pairs(live,seen):
            if a.r>b.r+1.2: a.r=min(44,a.r+2.2); b.alive=False
            elif b.r>a.r+1.2: b.r=min(44,b.r+2.2); a.alive=False
    def draw(d,Ww,Hh):
        _draw_arena(d,arena,cx,cy)
        for b in balls:
            if b.alive: _draw_ball(d,b)
...
def _swarm(cx,cy,W,H,shape,speed,palette,rng):
    ...
    def step(dt,hits):
        for b in balls:
            _move(b,dt)
            if _bounce_arena(b,arena,cx,cy,hits): n[0]+=1
        list(_collide(balls))
    def draw(d,Ww,Hh):
        _draw_arena(d,arena,cx,cy)
        for b in balls: _draw_ball(d,b)
...
def _rings(cx,cy,speed,palette,rng):
    radii=[95.0,155.0,215.0,275.0]; gaps=[rng.uniform(0.55,0.85) for _ in radii]
    ...
    def step(dt,hits):
        ...
        for k,R in enumerate(radii):
            ...
            dx,dy=ball.x-cx,ball.y-cy; dist=math.hypot(dx,dy) or 0.001
            if abs(dist-R)>ball.r+3: continue
            if abs(_wrap(math.atan2(dy,dx)-angles[k]))<gaps[k]/2: continue
            nx,ny=dx/dist,dy/dist; outward=dist>=R; dot=ball.vx*nx+ball.vy*ny
            if (outward and dot>0) or ((not outward) and dot<0):
                ball.vx-=2.18*dot*nx; ball.vy-=2.18*dot*ny; _boost(ball,1.08); hits[0]+=1
            side=1 if outward else -1
            ball.x=cx+nx*(R-side*(ball.r+2)); ball.y=cy+ny*(R-side*(ball.r+2))
        if math.hypot(ball.x-cx,ball.y-cy)>310: escaped[0]=True
    def draw(d,W,H):
        for k,R in enumerate(radii): _ring_gap(d,cx,cy,R,angles[k],gaps[k])
        _draw_ball(d,ball)
    return step,draw,lambda: escaped[0]

def _race(cx,cy,W,H,shape,speed,palette,rng):
    arena=_arena(shape,cx,cy,W,H); a=_mk(cx-70,cy-40,rng,palette,speed,12,0); b=_mk(cx+70,cy+40,rng,palette,speed,12,1); cap=arena.get("R",240)-8
    def step(dt,hits):
        for ball in (a,b):
            _move(ball,dt)
            if _bounce_arena(ball,arena,cx,cy,hits): ball.r=min(ball.r+2.6,cap)
        list(_collide([a,b]))
    return step, lambda d,Ww,Hh: (_draw_arena(d,arena,cx,cy),_draw_ball(d,a),_draw_ball(d,b)), lambda: a.r>=cap-1 or b.r>=cap-1
...
def _hit_polygon(ball,pts):
    ...
    for i in range(len(pts)):
        if _reflect_segment(ball,*pts[i],*pts[(i+1)%len(pts)]): hit=True
    return hit
...
def _reflect_segment(ball,x1,y1,x2,y2):
    ...
    if t<-0.02 or t>1.02 or abs(side)>ball.r+1.2: return False
    if side<0: nx,ny,side=-nx,-ny,-side
    dot=ball.vx*nx+ball.vy*ny
    if dot<0:
        ball.vx-=2.18*dot*nx; ball.vy-=2.18*dot*ny
        ball.x+=nx*(ball.r+1.2-side); ball.y+=ny*(ball.r+1.2-side); _boost(ball,1.08); return True
    return False
...
def _ring_gap(draw,cx,cy,R,mid,gap):
    ...
    for i in range(steps):
        ...
        if abs(_wrap((a0+a1)/2-mid))<gap/2: continue
